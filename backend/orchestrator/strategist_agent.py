...
def _design_spec_validation_error(content: str) -> str | None:
    ...
    # Core sections that must be present — relaxed matching:
    # Accept both roman numerals (I.) and arabic numerals (1.)
    # Accept partial title matches
    required = {
        "I": ["Project Information", "项目信息"],
        "II": ["Canvas Specification", "画布规格", "Canvas"],
        "III": ["Visual Theme", "视觉主题", "Color", "Theme"],
        "IX": ["Content Outline", "内容大纲", "Outline"],
        "XI": ["Technical Constraints", "技术约束", "Constraints"],
    }
    # Roman numeral → arabic mapping
    _roman_to_int = {"I": 1, "II": 2, "III": 3, "IV": 4, "V": 5,
                     "VI": 6, "VII": 7, "VIII": 8, "IX": 9, "X": 10,
                     "XI": 11, "XII": 12}
    for roman, titles in required.items():
        arabic = _roman_to_int.get(roman, "")
        found = False
        for title in titles:
            # Match "## I. Project Information" or "## 1. 项目信息" etc.
            pattern = rf"(?im)^#+\s*(?:{roman}|{arabic}\b)\.\s+.*{re.escape(title)}"
            if re.search(pattern, text):
                found = True
                break
            # Also accept the title alone without numeral prefix
            pattern2 = rf"(?im)^#+\s*.*{re.escape(title)}"
            if re.search(pattern2, text):
                found = True
                break
        if not found:
            return f"design_spec.md is missing section {roman}. {titles[0]}"
    return None
        ...
        if palette:
            ...
            if colors:
                override_lines.append(
                    ...
                    f"in every slide's color system."
                )
        if font:
            override_lines.append(
    ...
    for attempt in range(1, MAX_DESIGN_SPEC_ATTEMPTS + 1):
        ...
        try:
            response: LLMResponse = await asyncio.wait_for(
                ...
                timeout=180.0,  # 3 minutes per attempt
            )
        except asyncio.TimeoutError:
            last_error = f"LLM request timed out (attempt {attempt}/{MAX_DESIGN_SPEC_ATTEMPTS})"
            continue
